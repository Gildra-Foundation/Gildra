...
2. **Never invent game icons.** If a real asset exists, retrieve it. Spec/class icons live in `assets/specs/` and `assets/classes/` (official Wowhead CDN copies). The JS `GameAssets` resolver in index.html is the single source of truth for name → file mapping. Extend the resolver, don't scatter paths.
