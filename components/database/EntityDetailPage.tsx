...
export function EntityDetailPage({ entity, entityType, relationships, quality, versions, comparison, selectedFromBuildId, selectedToBuildId, lang }: Props) {
  ...
  return <>
    ...
    <div className="app"><main className="main"><article className="section route-section db-detail-page">
      <nav className="db-detail-breadcrumb" aria-label={lang === "ru" ? "Хлебные крошки" : "Breadcrumb"}>
        <Link href={`${prefix}/database`}>{lang === "ru" ? "База данных" : "Database"}</Link><span aria-hidden="true">/</span>
        <Link href={`${prefix}/database?type=${encodeURIComponent(entity.type)}`}>{entityType?.label ?? entity.type}</Link><span aria-hidden="true">/</span><span aria-current="page">#{entity.externalId}</span>
      </nav>

      <header className="db-detail-header">
        <div className="db-detail-title"><span className="db-detail-main-icon">{entity.iconUrl ? <img src={entity.iconUrl} alt="" /> : <svg className="i" aria-hidden="true"><use href={`#${entityType?.iconSymbol ?? "ic-gem"}`} /></svg>}</span><div><p className="cap gold">{entityType?.label ?? entity.type}</p><h1>{entity.name || `${entityType?.label ?? entity.type} #${entity.externalId}`}</h1>{description ? <p>{description}</p> : null}</div></div>
      ...
      <div className="db-detail-jump" aria-label={lang === "ru" ? "Разделы страницы" : "Page sections"}><a href="#tooltip">Tooltip</a><a href="#quality">{lang === "ru" ? "Качество" : "Quality"}</a><a href="#relations">{lang === "ru" ? "Связи" : "Graph"}</a><a href="#versions">{lang === "ru" ? "Версии" : "Versions"}</a></div>
