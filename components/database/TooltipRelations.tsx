...
export function TooltipOwners({ entries, lang }: { entries: Record<string, unknown>[]; lang: Lang }) {
	...
	const grouped = new Map<number, { name: string; icon: string; specs: Record<string, unknown>[] }>();
	for (const spec of specs) {
		...
		const group = grouped.get(classID) ?? {
			...
			icon: String(spec.class_icon_name ?? ""), specs: [],
		};
	...
	return <div className="db-tooltip-ownership">
		...
		<div className="db-tooltip-owner-paths">
			{Array.from(grouped.entries()).map(([classID, group]) => <div className="db-tooltip-owner-path" key={classID}>
				<OwnerBadge name={group.name || String(classID)} iconName={group.icon} />
				<span className="db-tooltip-owner-arrow" aria-hidden="true">→</span>
				<div className="db-tooltip-owner-specs">{group.specs.map((spec) => <OwnerBadge key={String(spec.owner_id)} name={String(spec.name)} iconName={String(spec.icon_name ?? "")} />)}</div>
			</div>)}
			{standalone.map((entry) => <OwnerBadge key={`${String(entry.owner_type)}-${String(entry.owner_id)}`} name={String(entry.name || fallbackClassName(Number(entry.owner_id), lang) || entry.owner_id)} iconName={String(entry.icon_name ?? "")} />)}
		</div>
	</div>;
}
...
export function RichDescription({ text, mentions, lang }: { text: string; mentions: Record<string, unknown>[]; lang: Lang }) {
	...
	while (offset < text.length) {
		...
		const icon = wowIconURL(String(selected.icon_name ?? ""));
...
function OwnerBadge({ name, iconName }: { name: string; iconName: string }) {
	const src = wowIconURL(iconName);
	return <span className="db-tooltip-owner-badge">{src ? <img src={src} alt="" loading="lazy" /> : null}<span>{name}</span></span>;
}

export function wowIconURL(iconName: string) {
	const icon = iconName.trim().toLowerCase();
	return /^[a-z0-9_]+$/.test(icon) ? `https://render.worldofwarcraft.com/us/icons/56/${icon}.jpg` : "";
}
