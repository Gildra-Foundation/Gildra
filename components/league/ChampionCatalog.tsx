...
export function ChampionCatalog({ champions, lang }: { champions: LeagueChampion[]; lang: Lang }) {
  ...
  const filtered = useMemo(() => champions
    ...
    .sort((a, b) => (sort === "asc" ? 1 : -1) * a.name.localeCompare(b.name, lang)), [champions, lang, query, role, sort]);
  ...
  return <>
    <div className={styles.controls}>
      <label className={styles.search}>
        ...
        <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={lang === "ru" ? "Найти чемпиона" : "Search champions"} />
        <kbd>/</kbd>
      </label>
