...
function descriptionStateLabel(state?: string) {
  if (state === "present") return "Готово";
  if (state === "unresolved") return "Не разрешено";
  if (state === "missing") return "Нет в источнике";
  return "Состояние не указано";
}

function descriptionStateClass(state?: string) {
  if (state === "present") return "border-[#367455] bg-[#0d2118] text-[#78d69d]";
  if (state === "unresolved") return "border-[#74622f] bg-[#211c0d] text-[#e1c36b]";
  return "border-[#744343] bg-[#211010] text-[#e18b8b]";
}
