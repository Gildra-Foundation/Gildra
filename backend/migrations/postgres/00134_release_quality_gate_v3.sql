-- +goose Up
-- The release quality gate (catalog_release_quality_gate_v1) is one SQL
-- statement whose planner estimate for the release candidates is a single
-- row, so every count CTE was joined with nested loops over 1.5M-row CTE
-- scans; publishing 12.1.0.69587 spent 18 hours inside it on 2026-09-05/06.
-- v3 stages every intermediate result in an analyzed temporary table so the
-- planner sees real cardinalities; the checks themselves are unchanged and
-- v1 stays available for comparison.
...
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION catalog_release_quality_gate_v2(p_release_id uuid)
RETURNS TABLE(check_key text, failed_count bigint, blocking boolean, details jsonb)
LANGUAGE sql
VOLATILE
AS $function$
    SELECT gate.check_key,gate.failed_count,gate.blocking,gate.details
    FROM catalog_release_quality_gate_v3(p_release_id) gate
    WHERE NOT (
...
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION catalog_release_quality_gate(p_release_id uuid)
RETURNS TABLE(check_key text, failed_count bigint, blocking boolean, details jsonb)
LANGUAGE sql
VOLATILE
AS $function$
...
$function$;
-- +goose StatementEnd
DROP FUNCTION IF EXISTS catalog_release_quality_gate_v3(uuid);
