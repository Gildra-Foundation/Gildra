...
func (s *Service) List(ctx context.Context, params ListParams) (Page, error) {
	...
	rows, err := s.postgres.Query(ctx, `
		...
		LEFT JOIN LATERAL (
			...
			|| COALESCE((SELECT jsonb_build_array(jsonb_build_object('type','item_effect_metadata','entries',jsonb_agg(jsonb_build_object('spell_id',effect.spell_id,'spell_name',effect.spell_name,'spell_target_status',effect.spell_target_status,'charges',effect.charges,'cooldown_ms',NULLIF(GREATEST(effect.cooldown_ms,0),0),'category_cooldown_ms',NULLIF(GREATEST(effect.category_cooldown_ms,0),0),'specialization_id',effect.specialization_id,'player_condition_id',effect.player_condition_id) ORDER BY effect.slot))) FROM catalog_item_effects effect WHERE effect.version_id=v.id AND (effect.charges<>0 OR effect.cooldown_ms>0 OR effect.category_cooldown_ms>0 OR effect.specialization_id<>0 OR effect.player_condition_id<>0 OR effect.spell_target_status='unavailable_in_build')),'[]'::jsonb) AS blocks
