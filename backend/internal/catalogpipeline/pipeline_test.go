...
func TestNormalizeOptionsSelectsProductAwareProfiles(t *testing.T) {
	tests := []struct {
		...
		{
			name: "classic profile rejects community source", product: "wow_classic", profile: ProfileClassicFoundation,
			sources: []string{"raidbots"}, buildVersion: "4.4.2.69900", wantErrContains: "outside the classic-foundation profile",
		},
	}
