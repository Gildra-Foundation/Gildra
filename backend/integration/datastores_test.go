...
func TestPostgresAndClickHouseMigrations(t *testing.T) {
	...
	var importRunsTable string
	if err := postgresDB.QueryRowContext(ctx, `SELECT to_regclass('public.catalog_import_runs')::text`).Scan(&importRunsTable); err != nil || importRunsTable != "catalog_import_runs" {
		t.Fatalf("catalog import migration failed: table=%q error=%v", importRunsTable, err)
	}
	var products int
